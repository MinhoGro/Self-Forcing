...
class CausalWanSelfAttention(nn.Module):
    ...
    def forward(
        ...
        if kv_cache is None:
            ...
            num_new_tokens = q.shape[1]  # roped_query/roped_key 的 token 数不变
            current_end = current_start + num_new_tokens

            old_local_end = kv_cache["local_end_index"].item()
            # print(f"kv_cache size: {kv_cache_size}, num_new_tokens: {num_new_tokens}, current_end: {current_end}, old_local_end: {old_local_end}")

            # 预测这一步为了塞下 num_new_tokens，会需要 evict 多少帧（与后面 eviction 分支保持一致）
            delta_frames_pred = 0
            num_evicted_tokens = 0
            if (current_end > kv_cache["global_end_index"].item()) and (num_new_tokens + old_local_end > kv_cache_size):
                raw_evicted_tokens = num_new_tokens + old_local_end - kv_cache_size
                delta_frames_pred = (raw_evicted_tokens + frame_seqlen - 1) // frame_seqlen
                num_evicted_tokens = delta_frames_pred * frame_seqlen

                # 不允许 evict sink，且保持帧对齐（与你后面代码一致）
                max_evictable = old_local_end - sink_tokens
                ...
                if num_evicted_tokens > max_evictable_aligned:
                    ...
                    delta_frames_pred = num_evicted_tokens // frame_seqlen

            # ✅ 新帧在“evict/roll 之后”的局部时间坐标落点（0-based）
            start_frame = (old_local_end // frame_seqlen) - delta_frames_pred
            if start_frame < 0:
                start_frame = 0
            ...
            if (current_end > kv_cache["global_end_index"].item()) and (
                ...
                delta_frames = delta_frames_pred
                # num_rolled_tokens = num_evicted_tokens
                num_rolled_tokens = old_local_end - sink_tokens - num_evicted_tokens
                ...
                # print(f"slide window is {sink_tokens + num_evicted_tokens}:{sink_tokens + num_evicted_tokens + num_rolled_tokens}, kv append at {local_start_index}")
            else:
